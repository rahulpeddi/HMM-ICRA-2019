...
where $\xi$ is a function of the lane separation, and $r_{q}^{l}(t)$ represents the risk associated with vehicle $q$, evaluated at lane $l$ at time $t$. As a result, \eqref{eq:riskcalc} returns risk values for each vehicle that are arranged as follows:
\begin{equation}r_q'=
\begin{bmatrix}
...
where $u_a$ and $u_h$ represent autonomous and human inputs, and $u$ is the input to the vehicle $h$. During run-time, we are monitoring $r(t+1),...,r(t+H)$ in order to find the appropriate pair of actions (velocity and lane) for $t+1$ that minimizes risk $r(t+H)$, in the event that $r(t+H)\geq r_\max$. We compute this pair by identifying the velocity and lane that produce the lowest risk based on our risk distributions $\hat{r}_v$:

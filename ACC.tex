...
%\NB{this is unnecessary. Say this before, above...we find the model $M^*$ that minimizes the error...}
  Given this model, we refer to its transition and emission matrices, $\mathcal{P}_{i^*}$ and $\mathcal{B}_{i^*}$, in order to make predictions. The procedure to predict future states is shown in Algorithm~\ref{alg:pred}. %\NB{something is missing here...we should somehow associate the error to the confidence that we have in that model....bigger errors = more uncertainties. The question becomes what should we do if the error is large or if it is small?? The answer is in the trust. What does it mean to trust mroe or less?}
...
\begin{equation} \label{eq:optpt}
  \mathcal{R}^* = \min\{\hat{\mathcal{R}}_{i1},\ldots,\hat{\mathcal{R}}_{in}\}  
\end{equation}
where $i = 1$. In other words, we are only concerned with the first row of $\hat{\mathcal{R}}$, which corresponded to $\tau$.
The value $\mathcal{R}^*$ gives information about the safest lane to enter and the safest velocity to reach. The optimal pair can be obtained by identifying $\mathcal{R}^*$ in the risk distribution, $\hat{\mathcal{R}}$:
\begin{align} \label{eq:optpair}
    l^* =  l_j\in\hat{L}\vert \mathcal{R}^* = \hat{\mathcal{R}}_{1j}^{v}\in\hat{\mathcal{R}} \nonumber \\
    v^* =  v\in\hat{V}\vert \mathcal{R}^* = \hat{\mathcal{R}}_{1j}^{v}\in\hat{\mathcal{R}}
\end{align}

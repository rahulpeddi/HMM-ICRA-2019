...
where $\hat{e} = \{e_i,\ldots,e_m\}$, and $i\in\mathbb{N}^m$. In \eqref{eq:pnorm}, we use the $l2$ matrix norm, which is defined as the sum of the absolute squares of the elements of the matrix, which in our case, represents the difference between transition matrices of the observed model and the offline models.

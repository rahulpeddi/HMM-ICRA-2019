...
where $\hat{e} = \{e_i,\ldots,e_m\}$, and $i\in\mathbb{N}^m$. In \eqref{eq:pnorm}, we use the $l2$ matrix norm
...
\begin{equation} \label{eq:optlane}
    l^* = l_i\in\hat{l}\vert l_i = \min(\hat{r}_q^l)
\end{equation}
\begin{equation} \label{eq:optvel}
    v_h^* = v_h\in\hat{v}\vert v_h = \min(\hat{r}_q^v)
\end{equation}

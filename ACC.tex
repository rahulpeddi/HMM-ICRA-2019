...
\begin{equation} \label{eq:optpt}
  r^* = \min\{\hat{\mathcal{R}}_{i1},\ldots,\hat{\mathcal{R}}_{in}\}  
\end{equation}
where $i = 1$. In other words, we are only concerned with the first row of $\hat{\mathcal{R}}$, which corresponds to $\tau$.
The value $\mathcal{R}^*$ gives information about the safest lane to enter and the safest velocity to reach. The optimal pair can be obtained by identifying $r^*$ in the risk distribution, $\hat{\mathcal{R}}$:

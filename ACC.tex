...
\end{enumerate}
where we assume that lanes are modeled in creasing order from left to right; i.e. the leftmost lane is $1$ and the rightmost lane is $n$.
...
\begin{equation} \label{eq:pnorm}
    \forall{P_i} \in \hat{\mathcal{P}}: e_i = \lVert\tilde{\mathcal{P}}-\mathcal{P}_{i}\rVert_{l2}
\end{equation}
...
where $\hat{e} = \{e_i,\ldots,e_m\}$, and $i\in\mathbb{N}^m$. In \eqref{eq:pnorm}, we use the $l2$ matrix norm, which is defined as the sum of the absolute squares of the elements of the matrix, which in our case, represents the difference between transition matrices of the observed model and the offline models. \NB{l2 of a matrix doesn't make sense if you are interested in obtaining only one element. Is there result of this operation a vector or 1 scalar? }
...
\begin{enumerate} %\NB{t+1 not i}
    ...
    \item $v_{h}^+(t+1)=v_h(t)+\delta_v$
\end{enumerate} 
and $\hat{v} = \{v_h^-,v_h,v_h^+\}$\NB{this set is time varying and depends on the current velocity}. Future reachable positions can be easily computed as $x_h(t+i)=v_h(t+i)\delta t$ for each of the three aforementioned scenarios. Reachable lanes are modeled as $\hat{l} = \{l_1,\ldots,l_n\}$, where there are $n$ lanes. 
...
where $n$ is the total number of vehicles in the sensing range. \NB{and what? What's the solution here? }
...
With the future velocities, we are able to calculate the forward positions for the horizon $H$ \NB{what you mean the position for the horizon? } using %\NB{no need to use the $\forall$ here}: 
...
such that risk increases as distance decreases \NB{what about the lanes?} and is calculated as follows:
...
where $\xi$ is a function of the lane separation \NB{what is this function?}, and $r_{q}^{l}(t)$ represents the risk associated with vehicle $q$, evaluated at lane $l$ at time $t$. As a result, \eqref{eq:riskcalc} returns risk values for each vehicle that are arranged as follows \NB{the vehicle are arranged as follows?}:
\begin{equation}r_q'=
\begin{bmatrix}
r^{l_1}(t+H) & \dots & r^{l_n}(t+H) \\
\vdots & \ddots & \\
r^{l_1}(t+1) &    & r^{l_n}(t+1)
\end{bmatrix}
\end{equation}
...
It is important to note that while multiple vehicles affect risk in our analysis, we are most concerned with the vehicle that causes the highest risk at any reachable point, as shown in Fig.~\ref{fig:reach} \NB{any reachable point shown in Fig.}. As a result, we identify the vehicle \NB{What you mean you identify the vehicle?} and its associated risk at each of these points as follows:%\NB{what's L? Define every variable, vector, set, etc...be careful that you may have already defined this before in another way}
\begin{equation} \label{eq:lanerisk} 
    \forall q\in R_h,\forall{l}\in{S_l}: \hat{r}_{v_h} = max(r_q')
\end{equation}
\NB{$\hat{r}_{v_h}$ is one value if I have to look this expression. why???}
 where $\hat{r}_{v_h} \in \mathbb{R}^{H\times n}$, where $H$ is the length of the time horizon and $n$ is the number of lanes. Using \eqref{eq:lanerisk}, we obtain a distribution of all of the highest risk values for velocity $v_h$. It is important to note\NB{don't use this expression too much} that \eqref{eq:riskcalc} and \eqref{eq:lanerisk} are calculated for each $v\in\hat{v}$ to obtain three different risk distributions; one for each velocity, $\hat{r} = \{\hat{r}_{v_h^-},\hat{r}_{v_h},\hat{r}_{v_h^+}\}$ \NB{how are you combining these risks in one risk?}. In Fig.~\ref{fig:riskd}, we show an example of a risk distribution over $H=3$ (left) for velocity $v_h$ at a the road scenario depicted (right). The magenta vehicle is the user's HAV, while the red and black vehicles are the actual positions of other vehicles on the road. Unfilled vehicles are each of the predictions in successive order; $[t+1,t+H]$.\NB{???}

\begin{figure}[ht]
    \includegraphics[width=0.48\textwidth]{riskdnew.png}
...
\includegraphics[width=\linewidth]{train1.png}
\caption{The forward position of the actual robot (red) is shown. After the training set, a prediction is made and the forward position of the estimate is shown (blue dashed).} \label{fig:train1}
\end{subfigure}
\hspace*{\fill} % separation between the subfigures
\begin{subfigure}{0.48\textwidth}
\includegraphics[width=\linewidth]{train2.png}
\caption{The lane changes of the actual robot (red) are shown. After the training set, a prediction is made and the lateral positions/lane changes of the estimate is shown (blue dashed).} \label{fig:train2}
\end{subfigure}
\caption{Simulation Results from Training Models} \label{fig:1}
\end{figure}

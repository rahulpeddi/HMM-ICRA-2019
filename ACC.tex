...
Hybrid Autonomous Vehicles (HAVs) are systems in which control authority is shared between a human user and an on-board computer. In such systems users can sometimes pass errant commands that compromise the integrity and safety of the system. In this paper, We develop a monitor capable of assessing and predicting the future states of surrounding vehicles, and correct the user's actions to ensure safety (i.e. no collisions). To achieve this, we propose a framework that uses Hidden Markov Model(HMM) theory and reachability analysis to predict and estimate the risk of future states, along with a method with which the autonomous controller will correct a user's action.
...
Building on the previous results, here we demonstrate the model fitting approach presented in Section...For the sake of simplicity in this work we trained and built 3 different vehicles models whose velocities was between $22$m/s and $32$m/s. Specifically one vehicle was aggressive, one was conservative, while the third vehicle's behavior was in between the other two behaviors. Three different transition and emission matrices were computed offline and used online to predict the behavior of a new vehicle with the assumption that this new vehicle's model was similar to one of the previously sampled behaviors. Following the procedure in Section \ref{sec:omf} at each iteration a model for the new vehicle is built and compared with each of the pre-trained models. The model with the minimum error (i.e. with the highest trust) is selected at each iteration to predict future states of the new vehicle.

...
In this figure, a user is driving a vehicle (yellow car) behind a slower vehicle (blue car) \NB{be consistent with the colors used in our simulation}. In this case, both actions presented in Fig.\ref{fig:hiway} may appear safe to a driver at the moment, but as can be seen by the red circle, one of the lane changes is considerably riskier in the future, because a rapidly approaching vehicle (red car) could enter the same space as the user's vehicle (i.e., creating a collision). In this work, we consider the driver's actions as a disturbance, and want to assist the driver\NB{and develop an approach to assist the driver during high risk situations}. In order to achieve this goal, we contribute a two-fold approach:
    \begin{itemize}
...
In future work, we will build more pre-trained models to increase richness. In this work, we are primarily concerned with. In addition, the state prediction method can be expanded across different types of vehicles and other cyber-physical systems.

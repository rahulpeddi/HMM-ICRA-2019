...
\begin{enumerate} %\NB{t+1 not i}
    ...
    \item $v_{h}^+(t+1)=v_h(t)+\delta_v$
\end{enumerate} 
and $\hat{v} = \{v_h^-(t),v_h(t),v_h^+(t)\}$, which is a time-varying set that depends on the current velocity, $v_h(t)$. Future reachable positions can be easily computed as $x_h(t+i)=v(t+i)\delta t$ for each of the three aforementioned velocities. Reachable lanes are modeled as $\hat{l} = \{l_1,\ldots,l_n\}$, where there are $n$ lanes. 
...
\begin{figure}[ht] \label{fig:critpt}
...
\begin{figure}[ht] \label{fig:noassist}
...
\begin{figure}[ht] \label{fig:assist}
    \includegraphics[width=0.48\textwidth]{assist.png}
    \caption{This figure depicts a situation in which assistance has occurred and a risky situation was avoided altogether.} 
\end{figure}

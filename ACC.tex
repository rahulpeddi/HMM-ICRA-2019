...
It is important to note that \eqref{eq:riskcalc} and \eqref{eq:lanerisk} are calculated for each $v\in\hat{v}$ to obtain three different risk distributions; one for each velocity.
...
\begin{equation} \label{eq:optlane}
    l^* = l_i\in\hat{l}\vert l_i = \argmin_{l_{i}}(\hat{r}_q^l)
\end{equation}
...
\begin{equation} \label{eq:optvel}
    v_h^* = v_h\in\hat{v}\vert v_h = \argmin_{v_{h}}(\hat{r}_q^v)
\end{equation}

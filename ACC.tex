...
and $M_{i^*}\in\mathcal{M}$ %\NB{i is a index not a model}
...
\begin{enumerate}
    ...
    \item $v_{h}^+(t+1)=v_h(t)+\delta_v$
\end{enumerate} 
\NB{t+1 not i}with $i=1,...,H$. Future reachable positions can be easily computed as $x_h(t+i)=v_h(t+i)\delta t$ for each of the three aforementioned scenarios.  
...
\NB{we need to define what are the inputs in our system} where $u_a$ and $u_h$ represent autonomous and human inputs, and $u$ represents the input to the vehicle $h$. During run-time, we are monitoring $r(t+1),...,r(t+H)$ in order to find the appropriate pair of actions (velocity and lane) for $t+1$ that minimizes risk $r(t+H)$. We compute this pair as follows
...
\end{equation}
The output pair $(l^*,v^*)= u_a$ \NB{this happens only when the risk is above a threshold}. In a situation where $u = u_a$, as defined in \eqref{eq:inputctrl}, the input to the HAV is the optimal lane and velocity as indicated by \eqref{eq:optlane} and \eqref{eq:optvel}. \NB{not clear}

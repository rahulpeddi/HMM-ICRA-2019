...
%along with a method with which the autonomous controller will correct a user's action.
The proposed approach is validated with simulations for an automotive setting with multiple vehicles having different behaviors.
...
\begin{figure}[ht!]
	...
	\subfigure[Road Scenario \label{fig:roads}]{\includegraphics[width=0.45\linewidth]{fig/riskdist_rs.png}}
	\vspace{-5pt}
...
\begin{subfigure}{\linewidth}
    ...
    \caption{Fitting error between the current vehicle and each of the three pretrained models}
    \label{fig:error}
\end{subfigure}
\begin{subfigure}{\linewidth}
    \includegraphics[width=\linewidth]{fig/trustfit.png}
    \caption{Trust associated with the best-fit model at each iteration}
    \label{fig:trust}
\end{subfigure}
\begin{subfigure}{\linewidth}
    \includegraphics[width=\linewidth]{fig/modelselect.png}
    \caption{The selected models during the operation}
    \label{fig:select}
\end{subfigure}

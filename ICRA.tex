...
%    In our case $\mathcal{O}$ represent measurements about velocity and positions of surrounding vehicles
    ...
    \item $\mathcal{P}\in\mathbb{R}^{n\times n}$ is a transition probability matrix. This matrix describes the probability of entering a certain state, $s_{j}$, while currently in a particular state $s_{i}$, denoted as $s_j \to s_i$, defined by the equation:
        ...
        \begin{equation} \label{eq:transbuild}
            p_{ij} = N_{ij}/N_{i}*
        \end{equation}
        where $N_{ij}$ represents the total number of transitions, $s_j \to s_i$, over $T$ and $N_{i*}$ is the total number of transitions from $s_i$ to any state. Also, $N_{ij} \leq N_{i*} \leq T$. The state transition matrix is right-stochastic, meaning the sum of all rows is $1$ and is of the form:
        \begin{equation}
            \mathcal{P} = 
                    \begin{bmatrix}
                        p_{11} & \dots & p_{1n} \\
...
This framework is executed over $T$ and a set of parameters is obtained: $\langle \mathcal{P}, \mathcal{B} \rangle$, with implicit parameters $m$ and $n$ \NB{this sentence is confusing}. The AHMM is different from a traditional HMM because the states are not hidden, and we know exactly the relationship
%\NB{clarify}
...
\end{equation}
    ...
    where $S_v$ is states pertaining to velocity and $S_d$ is a set of states pertaining to following distance between an agent $q$ and a preceding agent \NB{too many words...S is the set of velocities and S the set of distances between vehicles}. The emissions reflect the actions performed between two consecutive states. For velocity we consider the following 3 emissions,
...
For the lanes,  the distances in $S_d$ are used to identify when lanes change: $l(t) \neq l(t-1)$ \NB{this is not true}. The lanes themselves are modeled as states as well:
...
Algorithm \ref{alg:pred} \NB{summarized the steps...}shows how we carry forward this prediction through $H$.%\NB{fix this sentence}.
...
The following state, $s(t+2)$ is calculated using the previously determined $s_{j^*}$, assuming that it is the next observation. This process is repeated up to $t+H$, such that we have an series $\{s(t+1),\ldots,s(t+H)\}$. \NB{I think we can remove this if you have explained the procedure well before}
...
\STATE $t \gets t+1$
\FOR{$s(t) = s_i$}
\STATE $j* \gets \max_j(\mathcal{P}_{i}*)$ \RP{change row notation and explain what Pi* is}
\ENDFOR
...
As previously discussed, a series of prediction becomes less accurate as $H$ increases \NB{this is not correct! The prediction is still not sure because we don't know what's going to happen. On the other hand we want to improve the prediction itself and thus we update the model online}. In order to alleviate this issue, we propose a method to update $\mathcal{P}$ and $\mathcal{B}$ online. This is achieved by using \eqref{eq:transbuild} and \eqref{eq:obsbuild}, where we adjust $N_{ij}$ and $N_{ik}$, and therefore, $p_{ij}$ and $b_{ij}$ will change. As a result, new parameters $\mathcal{P'}$ and $\mathcal{B'}$ are created to reflect the updates. In addition, we use a sliding window approach such that the size of the training set is always a constant $T$. %\NB{is always constant $T$}\NB{remove this last part}.
...
where $M_{i^*}\in\mathcal{M}$ \NB{i is a index not a model}represents the model with the least error, or the optimal model. In order to make predictions using the optimal model, we refer to the parameters, $\mathcal{P}_{i^*}$ and $\mathcal{B}_{i^*}$. The procedure to predict future states is shown in Algorithm~\ref{alg:pred}. \NB{something is missing here...we should somehow associate the error to the confidence that we have in that model....bigger errors = more uncertainties. The question becomes what should we do if the error is large or if it is small?? The answer is in the trust. What does it mean to trust mroe or less?}
...
\begin{enumerate}
    ...
    \item $v_{h}^+(t+i)=v_h(t)+\delta_v$
\end{enumerate} with $i=1,...,H$ \NB{fix this}. Future reachable positions can be easily computed as $x_h(t+i)=v_h(t+i)\delta t$ for each of the three aforementioned scenarios.  
...
\end{equation}
where $v_q\in v_q^p$ and $\delta$ refers to a sampling time. \RP{Using 20} With this \NB{avoid these expressions} we obtain future forward positions:
\begin{equation}
...
Risk is a function of $d_q(t)$ \NB{and the lane} such that risk increases as distance decreases and is calculated as follows:
...
Using this \NB{avoid this notation, this what?}, we obtain a distribution of all of the highest risk values for each velocity.
...
Given the risk distribution, we can assist our user's potentially unsafe actions to guarantee safety \NB{formalize}. In order to adapt, we need to first identify whether these actions are going to result in a dangerous situation \NB{define dangerous condition}. A user-set range, $[r_\min,r_\max]$ is used as a threshold for risk. Given these values, our system intervenes as follows:\NB{...we propose a policy...}
\begin{equation}
    u = \begin{cases}
...
\end{equation}
The output pair $(l^*,v^*)$ represents $u_a$. In a situation where $u=u_a$, the input to the system is the optimal lane and velocity as indicated by \eqref{eq:optlane} and \eqref{eq:optvel}.
[insert lemma here where we prove that this always leads to the safest condition]

...
where $v_q^p$ and $l_q^p$ are the predicted future velocities and lanes for each vehicle $q$, respectively.
...
\begin{equation} \label{eq:dumpos}
    \forall{t}\in t+H: x_q(t) = v_q(t)\delta_t
\end{equation}
...
\end{equation}
Using this information, we are able build a risk profile and adapt the input of vehicle $h$ accordingly.
...
\begin{equation}
    \forall{l}\in\mathbf{S_l}:\hat{r}^l =  \max_{r_{q}^{l}}(\mathcal{R})
\end{equation}

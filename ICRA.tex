...
%    In our case $\mathcal{O}$ represent measurements about velocity and positions of surrounding vehicles
    ...
    \item  $\mathcal{S}\in\mathbb{R}^n$ is a finite set of $n$ unique values that $\mathcal{O}$ can obtain. $s_i,s_j \in \mathcal{S} \vert s_i \neq s_j, i\neq j$, where $i = \{1,\ldots,n$\}, with $n \in \mathbb{N}$.
    ...
    \item $\mathcal{C}$\NB{what's the dimension of C?} is the finite set of emissions, or inferences $c(t)$ that relate to the action taken each state. %\NB{what does it mean that it relates to each state? Explain better} $T$, $\mathcal{C} = \{ c(t-T), c(t-T+1), \ldots, c(t)\}$.  
    ...
    \item $\mathcal{P}\in\mathbb{R}^{n\times n}$ is a transition probability matrix. This matrix describes the probability of entering a certain state, $s_{j}$ at $t+1$, while currently in a particular state $s_{i}$ at $t$, defined by the equation:
        \begin{equation}

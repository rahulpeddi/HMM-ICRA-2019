...
\textbf{Problem 1: \textit{Proactive Safe Assisted Planning and Control}:} 
    ...
    A human-operated vehicle $i$ \NB{change $i$ with something else} is moving in an environment in the presence of stationary obstacles and other vehicles $q \in R_i(t)$, where $R_i(t)$ is a time varying set of vehicles in sensing/communication range with $i$. The goal is to find a policy to:
    \begin{enumerate}
    ...
    \end{equation}
    \item assess the risk $0\leq r_i \leq1$ of a collision during time horizon $T_u$ \NB{check} and
    \item assist and intervene to correct the human-operated actions to guarantee safety, i.e., obtain an input $u_i$ such that if $r_i>\rho, \forall q \in S$ \NB{define $\rho$ here or start the sentence with Given $\rho$ a user defined minimum risk threshold}and for \NB{for all t we can guarantee || x-x|| >dmin} 
    \begin{equation}
    t<=T |x_i(t)-x_q(t)| \geq d_{min}
    \end{equation}
     where $x_i(t)$ is the state of the human-operated vehicle at time $t$, $\rho$ is a user defined minimum risk threshold and $d_{min}$ is a minimum safe distance\NB{add $x_q$ definition}.
    \end{enumerate}
    
    It is important to note that the vehicle being modified is primarily human operated, in the sense that the approach to solving this problem should allow the user to operate the vehicle as she or he wishes, unless a prediction reveals that the user's action may lead to unsafe conditions, $r_i > \rho$ \NB{rephrase} \NB{we intervene only if necessary. Unless teh user performs actions that we predict could lead to unsafe conditions, we let the user perform his/her desired actions}.

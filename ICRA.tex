...
\begin{enumerate}
    ...
    \item $v_{h}^+(t+i)=v_h(t)+\delta_v$
\end{enumerate} with $i=\{1,...,H\}$. Future reachable positions can be easily computed as $x_h(t+i)=v_h(t+i)*\delta t$ for each of the three aforementioned scenarios. The set of reachable velocities is as follows: $\hat{v} = \{v_h^-,v_h,v_h^+\}$.
...
\end{equation}
[Can I put a 3D Matrix here? That might be a better way or should I say $\hat{r}\in\mathbb{R}^{l\times v}$]
Using these expression, we obtain a distribution of all of the highest risk values for each lane and for each velocity. [SHOW FIGURE BELOW and explain]

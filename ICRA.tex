...
MPC\cite{mpc} or QMDP\cite{qmdp} could be used to solve this problem as well, but in comparison to these methods, which require computationally expensive value iterations, this work is more efficient while guaranteeing safety
...
In this section we propose an HMM-based approach to predict future states of other vehicles, thus inferring their behavior to better assess safety and intervene to avoid hazardous situations. We leverage a history of offline observations to build different models that are used online to recognize and predict other vehicles' behaviors. Based on these predictions, we want to monitor whether our vehicle, $h$, enters an unsafe state. If such a state is detected, the proposed approach in Section X. %\NB{fix this}. %\NB{simplify}
...
The training set is kept the same size in order to retain computational efficiency and to avoid keeping older and less reliable data. Another option not used in this paper would be to discount older data, however we decide here not to use this method because the transition and emission matrices will grow larger, decreasing the efficiency of the proposed method. %\NB{simplify}
$\mathcal{P'}$ and $\mathcal{B'}$ are updated each iteration and will continue to be used for online prediction.  %\NB{rewrite}
...
In order to assess risk of collision we need to predict: i) future states of the surrounding vehicles which are obtained by following the procedure outlined in ... and ii) the reachable states of $h$ over the horizon $H$.
In some of our current work \cite{esen work} we are using Hamilton-Jacobi reachability analysis to predict future states of a system under uncertainties. In this work we consider a simplified approach for reachability in which we assume that: i) the vehicle can move to the adjacent lane in one time step $\delta t$ and ii) future variations of velocity are bounded $v_h-\delta_v \leq v_h\leq v_h+\delta_v$, with $\delta_v>0$ .
Three reachable sets are computed: 1) assuming $v_h(t+i)=v_h(t)$, 2) assuming $v_h(t+i)=v_h(t)-\delta_v$, and 3) assuming $v_h(t+i)=v_h(t)+\delta_v$, with $i=1,...,H$. Future reachable positions can be easily computed as $p_h(t+i)=v_h(t+i)*\delta t$ for each of the three aforementioned scenarios.  

...
\end{figure}
    ...
    In Fig.\ref{fig:hiway}, a vehicle is driving in a space where multiple actions appear safe at the moment. If the driver is unaware of the future risk of performing a certain action, an unsafe situation may arise, resulting in a collision. \NB{rephrase}
    \NB{We are interested in predicting future states of other vehicles, assess risk of collisions, and adapt the system accordingly} \NB{We leverage the HMM theory }
    Within this work, we are interested in finding a way to determine whether or not this driver's actions are indeed unsafe, and how the vehicle itself, assuming it is capable of some autonomy, can intervene and adapt to the situation at hand. This type of work can be approached as a type of Markov Chain, which is a commonly used model for many cyber-physical systems. The proposed model in this work is closely related to a Hidden Markov Model \NB{no need of name here}, where the states are hidden, but we are able to make observations \NB{still confusing}. In this paper, the proposed model is named \textit{Adjusted Hidden Markov Model}, where we have the benefit of the states no longer being hidden, and being able to draw a direct connection between states and observations at those states, which ultimately will help predict future states \NB{confusing}. 
    ...
    \item{an adaptive framework that determines the adjustment and severity of such adjustment that should be made at any point in time\NB{be specific that we are adjusting actions}}
    \end{itemize}
  
    These problems are solved using our proposed framework \NB{no need for these sentences}, and are specifically implemented in a highway-type situation, where there are multiple vehicles performing different behaviors. \NB{Although the proposed approach is applied in the context of autonomous and semi-autonomous vehicles, it is general and applicable to any other CPS}
    
\NB{stress that we see the user as a disturbance or adversarial effect. We monitor the user behavior and intervene if necessary by predicting future states of our system and the surrounding entities (obstacles and other vehicles)}    
...
In this work we are interested in finding a method to proactively guarantee safety (i.e., something bad will never happen) by adapting a human user's actions and inputs into a semi-autonomous mobile system \NB{remove adapting...and discuss the general problem}. \NB{In this work we are interested in finding an approach to proactively guarantee safety (i.e., something bad will never happen) in multi-vehicle systems in which users may perform undesired actions that can compromise the safety of the entire system} Formally: 
...
\textbf{Problem 1: \textit{Proactive Safe Assisted Planning and Control}:} 
    ...
    It is important to note that the vehicle being modified is primarily human operated, in the sense that the approach to solving this problem should allow the user to operate the vehicle as she or he wishes, unless a prediction reveals that the user's action may lead to unsafe conditions, $r_i > \rho$ \NB{rephrase} \NB{we intervene only if necessary. Unless the user performs actions that we predict could lead to unsafe conditions, we let the user perform his/her desired actions}.

\section{Approach}
\NB{add the general approach here}
\subsection{Adjusted Hidden Markov Model}
In this section, we outline a novel model that can be used to accurately predict the future states of a system \NB{rephrase as In this section we propose a novel HMM-based approach to predict future states of other vehicles. We leverage an history of offline observations to build different models that are used online to recognize and predict other vehicles behaviors}. The technique derived in this work also aims to find a method to guarantee safe operation based on the state observations and predictions that come from this model. MPC [ref] or QMDP [ref] could be used to solve this problem as well, but those two processes feature unneeded considerations and complications \NB{...be specific talk about the computation complexity}. The AHMM based approach provides a less computationally expensive method to guarantee safe operation. \NB{In comparison to MDP and QMDP that require computationally expensive value iterations, our propose work is more efficient while guaranteeing safety.} \NB{IMPORTANT: Introduce the training set concept: sates are observed offline to extract a model of a vehicle. Multiple vehicles are sampled to extract different models that capture the behavior of the system}

\subsubsection{Framework}

The proposed Adjusted Hidden Markov Model can be described by a tuple $\langle \mathcal{O},\mathcal{E},\mathcal{A},\mathcal{B},\rangle$, \NB{add $\mathcal X$} where:
\begin{itemize}
    \item $\mathcal{S}$ is a finite set of states $s_i$ collected over a finite past time horizon $T$, $\mathcal{S} = \{ s_{t-T}, s_{t-T+1}, \ldots, s_{t}\} $ \NB{need to add the $n$ possible values that $s$ can be, pulled from a finite set $\forall s(t)\in\mathcal{S}$, $s(t) \in \mathcal{X}$ where $\mathcal{X}$}
    
    denoted as $s_{i}$ or $s_{j}$, where $i$ and $j$ are in $\mathbb{N} \leq n$. The notation $s_{ij}$ represents the state transition from $i$ to $j$.
    ...
    \item $\mathcal{P}$ is a transition probability matrix with dimension $n \times n$. This matrix describes the probability of entering a certain state, $s_{i}$ at the next time-step, while currently in a particular state $s_{j}$, defined by the equation:
        \begin{equation}
            p_{ij} = P(s_j \text{ at } t+1 \vert s_i \text{ at } t)
        \end{equation}
...
\NB{let's use S for measurements, O for observations, X for state, P for transition prob, B for emission}


\NB{stress about training sets, each training set is about a certain vehicle that you are observing over an horizon T } This framework is executed over $T$ and a set of parameters is obtained: $\langle \mathcal{A}, \mathcal{B} \rangle$, with implicit parameters $m$ and $n$. The AHMM is adjusted from a traditional Hidden Markov Model because the states are are measurable. The benefit we draw with measurable states is that the connection between states and observations are not hidden, and therefore, predictions can be made with increased accuracy. \NB{the only difference is that we have all the states and transitions a priori and can learn a model offline}
...
\subsubsection{Making Predictions with the AHMM}\NB{change titles...HMM-based Prediction}
 Using the framework above, we build a model of a certain system offline. After $T$, the parameters of the AHMM can be used to make online predictions about the future states of the observed system. This is done by identifying the current state, $s_{i,T}$ \NB{and applying to the transition and emission matrices to obtain the next most likely state and its likelihood...talk about lookup table to find the prediction} and applying the parameters: We start with the assumption that $\mathcal{A}$ will give conclusive results \NB{add this consideration and with more details later }:
...
\NB{Using this framework it is possible to predict over any future horizon H asuuming that the prediction is correct at each iteration and considering at every iteration the results from the transition and emission matrices} \NB{add algorithm}

\subsubsection{Online Updating and Training of AHMM Parameters} \NB{add sliding window approach to always have T observations and remove old unneccessary}
...
\NB{Using the framework described in Sections 1 2 and 3  we can extract a model that capture the behavior of a system that we have been observing for T. Multiple models can be extracted using several data sets about different systems under observation. In other words we can obtain add super-set containing all the transition and emission matrices}.\NB{During run-time we observe new measurements about other systems and the challenge consists on fitting the new system observations to the right model} \NB{the challenge here is that until we have enough data the prediction may not be correct and thus we need to tae into account this} In efforts to minimize situations where transitions are unclear, we propose training multiple models prior to the prediction process, and also a method to ensure that the correct model is used in the prediction \NB{later...explain that the more models the more precise and likely you are to find a good fit}. Let us build 3 models, $M_1,M_2,M_3$ to represent some system that behaves in 3 different ways. Using the method outlined in this section, we are able to determine and apply the best model.
...
{\begin{tabular}{ |c||c||c|c|c||c|  }
 \hline
 \multicolumn{6}{|c|}{Updating $M^*$} \\
 \hline
 Time& $M^*(t)$& $P_{M_1}$& $P_{M_2}$& $P_{M_3}$& $M^*(t+1)$\\
 \hline
 $t_1$&$M_3$&$P_{M_1}-\frac{\gamma}{M-1}$& $P_{M_2}-\frac{\gamma}{M-1}$&$P_{M_3}+\gamma$&$M_2$\\
 \hline
 $t_2$&$M_2$&$P_{M_1}-\frac{\gamma}{M-1}$&$P_{M_2}+\gamma$&$P_{M_3}-\frac{\gamma}{M-1}$& $M_2$\\
 \hline
 \vdots & \vdots & \vdots& \vdots&\vdots&\vdots\\
 \hline
 $T_u$&$M_n$&$P_{M_1}$&$P_{M_2}$&$P_{M_3}$&$M_n$\\
 \hline
\end{tabular}}
\caption{This table shows how we update $M^*$ at each $t$}
 \label{tab:probup}
\end{table}

While this solution is viable, there is an increased possibility of having drastically incorrect predictions until the model is rich enough to effectively describe the observed system.
...
\begin{equation}
    u_a > u_h \iff \hat r^{l^*} > b
\end{equation}
where $u_a$ and $u_h$ represent autonomous and human control, while $l^*$ represents the user's lane. In the case that the autonomous control input surpasses that of the user input, the system analyzes the correct action to take. Because the risk is calculated for each lane, the initial action is to identify if a certain lane has a lower risk. With this, we identify $l^*$, which is the lane with the lowest risk. However, we do not immediately default to changing the lane. Let us take, for example, our host vehicle travelling at a much higher velocity than a preceding agent in the same lane. Our system identifies that our user has not shown any sign of turning, and because of that, we assume the user's priority is to stay in the lane. As a result of this, the $u_a$ involves lowering the velocity first. Assuming that the user doesn't respond, our system will continue to lower the velocity such that $u_s = v_q$, as long as the risk remains below $\rho$. If the user respond with a sub-optimal lane change, then the system will assist the user by reverting to the optimal action instead.  
